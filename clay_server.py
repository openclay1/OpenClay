...
import http.server, io, json, re, subprocess, sys, threading, time, email.parser
import hashlib, glob as glob_module, uuid, socket as _socket, webbrowser
...
def _task_run(task_id):
    ...
    task["status"] = "running"
    _task_save(task)
    _log_write("system", f"Task started: {task['goal'][:100]}", _model or "none")
...
_BUILD_VERBS = {"build", "create", "make", "generate", "write", "code", "develop", "give", "show"}
_BUILD_NOUNS = {"app", "website", "web", "site", "page", "landing", "script", "program",
                "tool", "game", "dashboard", "calculator", "form", "todo", "api",
                "server", "counter", "clock", "timer", "quiz", "chat", "portfolio"}


def _is_build_intent(prompt: str) -> bool:
    words = set(re.findall(r'\w+', prompt.lower()))
    return bool(words & _BUILD_VERBS) and bool(words & _BUILD_NOUNS)


def _slugify(text: str) -> str:
    text = re.sub(r'[^\w\s-]', '', text.lower().strip())
    text = re.sub(r'[\s_-]+', '-', text)
    parts = [w for w in text.split('-') if w][:5]
    return '-'.join(parts) or "project"


def _get_or_create_workspace(prompt: str) -> str:
    """Derive project slug from prompt, create ~/OpenClay/projects/<slug>/, return path."""
    cleaned = re.sub(
        r'\b(build|create|make|generate|write|code|me|a|an|the|for|with|using|simple|basic|quick|small)\b',
        '', prompt, flags=re.IGNORECASE
    ).strip()
    slug = _slugify(cleaned) or "project"
...
def _project_type(prompt: str) -> str:
    p = prompt.lower()
    if any(w in p for w in ("python", " script", "cli", "command line", "terminal", "data analysis", "csv")):
        return "python"
    if any(w in p for w in ("landing page", "static page", "html only", "html page")):
        return "html"
    return "webapp"
...
def _write_templates(ptype: str, project_path: str) -> list:
    """Write deterministic starter files — no model required."""
    p = Path(project_path)
    title = p.name.replace("-", " ").title()

    if ptype == "python":
        (p / "main.py").write_text(
            f'#!/usr/bin/env python3\n# {title}\n# Generated by ClayCode\n\n'
            f'def main():\n    print("Hello from {title}!")\n'
            f'    print("Edit main.py to customize.")\n\n'
    ...
    if ptype == "html":
        (p / "index.html").write_text(
            ...
            f'<title>{title}</title>\n'
            f'<style>\n'
            f'  body {{ font-family: system-ui, sans-serif; max-width: 800px; margin: 4rem auto; padding: 0 2rem; background: #fafafa; }}\n'
            f'  h1 {{ color: #e06438; }}\n'
            f'  p {{ color: #555; line-height: 1.6; }}\n'
            f'</style>\n</head>\n<body>\n'
            f'<h1>{title}</h1>\n'
            f'<p>Your page is running. Edit <code>index.html</code> to get started.</p>\n'
            f'</body>\n</html>\n',
            encoding="utf-8"
        )
        return ["index.html"]
    ...
    (p / "index.html").write_text(
        ...
        f'  <h1>{title}</h1>\n'
        f'  <p>Your app is running. Edit the files to build it out.</p>\n'
        f'  <button id="btn">Click me</button>\n'
        f'  <div id="output"></div>\n'
        f'</div>\n'
    ...
    (p / "style.css").write_text(
        ...
        ' display: flex; justify-content: center; padding: 4rem 2rem; }\n'
        '.container { background: white; border-radius: 12px; padding: 2rem;'
        ' max-width: 600px; width: 100%; box-shadow: 0 4px 16px rgba(0,0,0,0.1); }\n'
        'h1 { color: #e06438; margin-bottom: 1rem; }\n'
        'p { color: #666; margin-bottom: 1.5rem; line-height: 1.6; }\n'
        'button { background: #e06438; color: white; border: none;'
        ' padding: 0.75rem 1.5rem; border-radius: 999px; cursor: pointer; font-size: 1rem; }\n'
        'button:hover { background: #d05025; }\n'
        '#output { margin-top: 1rem; font-weight: 500; }\n',
        encoding="utf-8"
    )
    (p / "script.js").write_text(
        'document.getElementById("btn").addEventListener("click", function() {\n'
        '  document.getElementById("output").textContent = "It works! \U0001f389";\n'
        '});\n',
        encoding="utf-8"
    )
...
def _generate_project_files(prompt: str, project_path: str) -> list:
    """Generate files with model when available; deterministic templates otherwise."""
    ptype = _project_type(prompt)
    if _model is None:
        return _write_templates(ptype, project_path)
    files_spec = {"python": "main.py", "html": "index.html"}.get(ptype, "index.html, style.css, script.js")
    gen_prompt = (
        f"Generate the following files for this request: {prompt}\n\n"
        f"For EACH file use EXACTLY this format:\n"
        f"=== FILENAME: <name> ===\n<full file content>\n=== END ===\n\n"
        f"Files to generate: {files_spec}\n"
        f"Write complete, working, self-contained code. No explanations. No extra text."
    )
    try:
        response = _builder_ollama_call(
            gen_prompt,
            system="You are a code generator. Output ONLY the requested files using the exact === FILENAME === format. No explanations."
        )
...
def _find_free_port(start: int = _BUILD_PORT_START) -> int:
    port = start
...
def _run_project(project_path: str) -> dict:
    ...
    p = Path(project_path)
    slug = p.name
    # Reuse existing live server
    existing = _build_servers.get(slug)
    if existing and existing.get("proc") and existing["proc"].poll() is None:
        return {"url": f"http://localhost:{existing['port']}", "output": ""}
    # HTML → serve with http.server
    if (p / "index.html").exists():
        port = _find_free_port()
        proc = subprocess.Popen(
            [sys.executable, "-m", "http.server", str(port), "--bind", "127.0.0.1"],
            cwd=str(p), stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
            start_new_session=True
        )
        _build_servers[slug] = {"port": port, "proc": proc}
        time.sleep(0.5)
        return {"url": f"http://localhost:{port}", "output": ""}
    # Python script → run and capture output
    if (p / "main.py").exists():
        try:
            r = subprocess.run(
                [sys.executable, "main.py"], cwd=str(p),
                capture_output=True, text=True, timeout=15
            )
            return {"url": "", "output": (r.stdout + r.stderr).strip() or "(no output)"}
        except subprocess.TimeoutExpired:
            return {"url": "", "output": "Script ran but timed out (15s limit)."}
        except Exception as exc:
            return {"url": "", "output": str(exc)}
    # npm project
    if (p / "package.json").exists():
        ...
        time.sleep(2)
        return {"url": f"http://localhost:{port}", "output": ""}
    return {"url": "", "output": "No runnable entry point found in project."}
...
class ClayHandler(http.server.SimpleHTTPRequestHandler):
    ...
    # ── Fallback: instant rule-based response when no model is available ──
    def _fallback_response(self, prompt: str) -> str:
        ...
        if "sort" in p and "list" in p:
            return "my_list = [3, 1, 4, 1, 5]\nmy_list.sort()          # in-place\nsorted_list = sorted(my_list)  # new list"
        if "read file" in p or "open file" in p:
    ...
    def _send_fallback_stream(self, text: str) -> None:
        ...
        self.send_header("Access-Control-Allow-Origin", "*")
        self.end_headers()
        self.wfile.write((json.dumps({"response": text, "done": True}, ensure_ascii=False) + "\n").encode())

    def _handle_build(self, prompt: str) -> None:
        """ClayCode builder loop: generate files → run → open browser → stream progress."""
        ...
        try:
            files_written = _generate_project_files(prompt, project_path)
            source = "model" if _model else "template"
            for fname in files_written:
                _emit(f"\U0001f4c4 `{fname}` ({source})\n")
        except Exception as exc:
            _emit(f"\u26a0\ufe0f File generation error: {exc}\n")
            files_written = []
        ...
        if result.get("url"):
            ...
            _emit(f"\U0001f4c1 Files saved to: `{project_path}`\n")
            summary = f"Files written to `{slug}`"
    ...
    def _handle_status(self):
        ...
        self._send_json({
            ...
            "new_ingested": count, "model": _model or "",
